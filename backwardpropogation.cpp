...
Convnet CNNBP(Convnet net, Mat y, Mat x)
{
  ...
  //cout << "\n" << net.e;
  Mat te;
	// derivative for sigmoid: od = net.y * (1-net.y) 
//  subtract(1, net.output, te);
//  multiply(net.output, te, net.od);
	//derivative for relu: od = wfv > 0 ? 1 : 0; where wfv is net.y before relu
	//dimen: 10 x 1
	Mat::MSize wfv_s = net.wfv.size;
	//dimen: 10 x 1
//cout<<"wfv_s[0]: "<<wfv_s[0]<<" and wfv_s.rows: "<<net.wfv.rows<<endl;
	net.od = Mat(wfv_s[0], wfv_s[1], net.wfv.type());	

	for(int r =0; r<wfv_s[0]; r++){
		double* p = net.od.ptr<double>(r);
		double* q = net.wfv.ptr<double>(r);
		for(int c = 0; c<wfv_s[1]; c++){
			*p = (*q) > 0 ? 1 : 0;
			p++;
			q++;
		}
  ...
  //cout << "\n" << net.od;
		//dimen: 10 x (16*64) //dimen:  (16*64)x10
  transpose(net.ffw, temp1);
	//((16x16)*10) * 10*50
// dimen: (16*64) * 1
  net.fvd = temp1 * net.od;

  Mat::MSize sa = net.layer3.a[0].size;
  int channels = net.layer3.a[0].channels();
  int fvnum = sa[0] * sa[1];
  ...
  {
	//Error(layer 3) = T(weights(layer Out))*error(layer Out) .* g'(z(layer3))
	//				 = fvd * g'(z(layer3))
			//Originally: First iter range from row 0 to 16, all 50 coloums
	Mat temp1;
	Mat temp = net.fvd( Range((j)*fvnum, (j+1)*fvnum), Range::all());
	temp = temp.reshape(channels, sa[0]); //temp = fvd for jth layer
	
	//Obtain activation derivative
  	subtract(1,net.layer3.a[j],temp1);
  	multiply(net.layer3.a[j], temp1, temp1);
